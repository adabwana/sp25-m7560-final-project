...
#Be sure to run first part first to obtain 'maintrain'
...
for (i in 1:5) {
  ...
  tcF <- trainControl(method = "cv", number = 5, search = "grid")
  #Adjust as needed
  CVF <- train(Duration_In_Min ~ ., data = maintrain, method = "earth",
               trControl = tcF, tuneLength = 3)
  #Adjust as needed
  
  fittedF <- predict(CVF, newdata = maintrain[testInd[[i]],])
