...
def objective(trial: optuna.trial.Trial,
    ...
    # Tunable (Refined based on previous Occupancy results)
    lr = trial.suggest_float("lr", 8e-4, 1.5e-3, log=True) # Narrowed range
    weight_decay = trial.suggest_float("weight_decay", 1e-6, 1e-5, log=True)
    batch_size = trial.suggest_categorical("batch_size", [64, 128]) # Focused range
    d_model = trial.suggest_categorical("d_model", [64, 128, 256]) # Keep exploring
    dropout = trial.suggest_float("dropout", 0.05, 0.20) # Kept narrowed range
    nlayers = trial.suggest_int("nlayers", 2, 3) # Kept narrowed range
    ...
    # Ensure d_model is divisible by nhead (Pruning approach)
